...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth, height=0.3\textheight, keepaspectratio]{images/Llama-3.1-8B-Instruct_belief_bank_facts_activations.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth, height=0.3\textheight, keepaspectratio]{images/Gemma-2-9B-IT_belief_bank_facts_activations.png}
...
Per gli esperimenti successivi sono stati scelti per ogni componente i 3 layer migliori in termini di performance.
\begin{table}[H]
\centering
...
\begin{itemize}
\item \textbf{AlignmentNetwork \ref{fig:alignmentnetwork-architecture-full}:} Viene utilizzata la stessa architettura descritta nell'approccio precedente con la stessa loss
\item \textbf{MLP Prober \ref{fig:mlp-prober-architecture-full}:} Classificatore non-lineare per rilevamento della allucinazione: rete fully-connected con layer intermedi normalizzati (LayerNorm), attivazione GELU e dropout.
\item \textbf{BCEWithLogitsLoss:} combina una funzione sigmoide e la Binary Cross Entropy per stabilità numerica.
\end{itemize}

\subsubsection{Pipeline sperimentale}
La pipeline sperimentale \ref{fig:non-linear-complete-pipeline} è dunque identica a quella dell'approccio ibrido, con la differenza che il classificatore sul teacher è ora un MLP Prober non-lineare.
