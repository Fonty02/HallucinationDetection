...
\begin{quote}
    You are an helpful assistant that helps to identify hallucinations.
...
\textcolor{blue}{Qui non ho sbagliato io, per LLama manca qualche attivazione}
...
\begin{itemize}
\item \textbf{Dataset Concordante (per Allineamento):} Si selezionano solo gli esempi in cui Trainer e Tester concordano sulla classificazione (stessa label). Su questi viene applicato un undersampling per bilanciare le classi. Questo è essenziale per apprendere una mappatura coerente tra tester e trainer
...
\end{figure}
Alcune caratteristiche chiave dell'AlignmentNetwork includono: \textcolor{blue}{chiamata io cosi}
\begin{itemize}
...
\textcolor{blue}{Non sto ri-addestrando qualcosa per allineare. L'autoencoder è uno per ogni modello, la componente encoder la uso perchè l'idea di partenza è che riduca il rumore e migliori l'allineamento. Quindi poi vado ad allineare questi spazi encodati}
...
\textcolor{blue}{La classification head la addestro una sola volta con il trainer, poi la congelo e la riutilizzo per lo tester. Quindi si: se la riutilizzo sul trainer mantiene gli stessi risultati}
