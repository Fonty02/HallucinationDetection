...
for i, layer in enumerate(layers):
    ...
    ax.bar(x + offset, data_trainer[:, i], width, label=f'{layer} (Tr)', color=colors_teacher[i])

# Plot student bars (lighter, adjacent to teacher)
for i, layer in enumerate(layers):
    offset = (i - 1) * width * 2 + width/2
    ax.bar(x + offset, data_tester[:, i], width, label=f'{layer} (Te)', color=colors_student[i], edgecolor=colors_teacher[i], linewidth=1)

ax.set_xticks(x)
ax.set_xticklabels(domains, rotation=33, ha="center", fontsize=18, fontweight='bold')
...
ax.legend(title="Layer (Model)", fontsize=22, ncol=1, loc='center left', bbox_to_anchor=(1.01, 0.5),
          prop={'weight': 'bold', 'size': 20}, title_fontproperties={'weight': 'bold', 'size': 22})
