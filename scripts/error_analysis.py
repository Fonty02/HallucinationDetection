...
def plot_error_type_breakdown_by_dataset(df: pd.DataFrame, models: list,
                                          plots_dir: Path) -> None:
    datasets = sorted(d for d in df["dataset"].dropna().unique() if d != "unknown")
    if not datasets:
        return
    for dataset in datasets:
        ...
        if sub_df.empty:
            continue
        et_counts = (
            sub_df.groupby(["model", "layer", "error_type"])
            .size()
            .reset_index(name="count")
        )
        fig, axes = plt.subplots(1, len(models), figsize=(7 * len(models), 5), sharey=False)
        if len(models) == 1:
            axes = [axes]
        for ax, model in zip(axes, models):
            pivot = et_counts[et_counts["model"] == model].pivot(
                index="layer", columns="error_type", values="count"
            ).fillna(0)
            if pivot.empty:
            ...
            pivot.plot(kind="bar", ax=ax,
                       color=[PALETTE.get(c, "grey") for c in pivot.columns],
                       stacked=False, rot=0, width=0.7)
            ax.set_title(model)
            ax.set_ylabel("Count")
            ax.legend(title="Error type")
        fig.suptitle(f"Dataset: {dataset}", fontsize=12, y=1.02)
        plt.tight_layout()
        _savefig(fig, plots_dir, f"error_type_breakdown__{_slug(dataset)}")
...
def run_analysis(df: pd.DataFrame, args: argparse.Namespace) -> None:
    ...
    plot_error_type_breakdown_by_dataset(df, models, PLOTS_DIR)
