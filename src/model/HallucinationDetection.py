...
class HallucinationDetection:
    ...
    def save_activations(self):
        ...
        for batch_idx in range(num_batches):
            ...
            for idx in tqdm(
                range(start_idx, end_idx), desc=f"Batch {batch_idx + 1}/{num_batches}"
            ):
    ...
    def combine_activations(self):
        ...
        for aa in self.ACTIVATION_TARGET:
            act_dir = os.path.join(
                results_dir, model_name, self.dataset_name, f"activation_{aa}"
            )
            ...
            for layer_id in self.TARGET_LAYERS:
                ...
                ids_save_path = os.path.join(
                    act_dir, f"layer{layer_id}_instance_ids.json"
                )
    ...
    def _create_folders_if_not_exists(self):
        ...
        self.mlp_save_dir = os.path.join(
            results_dir, model_name, self.dataset_name, "activation_mlp"
        )
        self.attn_save_dir = os.path.join(
            results_dir, model_name, self.dataset_name, "activation_attn"
        )

        self.generation_save_dir = os.path.join(
            results_dir, model_name, self.dataset_name, "generations"
        )
        self.logits_save_dir = os.path.join(
            results_dir, model_name, self.dataset_name, "logits"
        )

        for sd in [
            self.hidden_save_dir,
            self.mlp_save_dir,
            self.attn_save_dir,
            self.generation_save_dir,
