...
def plot_source_experiment_mean(
    ...
    plt.title(f"{source_experiment} [{title_suffix}] - Mean {metric} across seeds")
    ...
    output_path = output_dir / f"{safe_name(source_experiment)}_{metric}_mean.pdf"
...
def main() -> None:
    ...
    mean_df = (
        long_df.groupby(
            ["source_experiment", "encoder_experiment", "dataset_activation", "layer_type", "split", "layer_split"],
            as_index=False,
        )["score"]
        .mean()
        .sort_values(["source_experiment", "layer_type", "split"])
    )

    args.out_dir.mkdir(parents=True, exist_ok=True)
    experiments = sorted(mean_df["source_experiment"].unique().tolist())

    for source_experiment in experiments:
        subset = mean_df[mean_df["source_experiment"] == source_experiment]
        if subset.empty:
            continue
        plot_source_experiment_mean(
            source_df=subset,
    ...
    print(f"Generated {len(experiments)} cross-domain mean plots in: {args.out_dir}")
