...
# make truthx/ available for importing MLP.py
sys.path.append(os.path.join(os.path.dirname(__file__), "truthx"))
from MLP import MLP
    ...
    for instance_id in sorted_ids:
        for layer_idx, module_type in layer_type_pairs:
            ...
            if key in layer_data and instance_id in layer_data[key]:
                ...
                raise ValueError(
                    f"Missing activation for instance {instance_id}, layer {layer_idx}, type {module_type}"
                )
...
def train_mlp(X_train, y_train, X_val, y_val, input_size, output_size, device,
              epochs=1000, patience=50, lr=1e-3, batch_size=512, hidden_size=1024):
...
if __name__ == '__main__':
    ...
    for aligner_name, aligner_info in ALIGNERS.items():
        print(f"\nProcessing MLP2 aligner: {aligner_name}")
        out_base = os.path.join("alignment_cache_mlp2", aligner_name)
