...
def find_csv_path() -> Path:
    candidates = [
        Path("truthXEperiments.csv"),
        Path(__file__).resolve().parents[2] / "truthXEperiments.csv",
    ]
    for path in candidates:
        if path.exists():
            return path
    raise FileNotFoundError("truthXEperiments.csv non trovato.")


def source_dataset_from_row(row) -> str | None:
    ae_path = str(row.get("truthX_autoencoder", "") or "")
    match = re.search(r"AutoEncoder/([^/]+)/", ae_path)
    if match:
...
def normalize_experiment(row) -> tuple[str, str]:
    ...
    if "truthx" in exp_type or "stanadard" in exp_type:
        return "truthx_standard", CATEGORY_LABELS["truthx_standard"]

    if exp_type == "cross-dataset":
    ...
    if exp_type in {"crossmodel", "cross_model1", "cross_model2", "cross_model"}:
